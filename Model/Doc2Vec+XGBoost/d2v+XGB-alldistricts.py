...
def doc2vec(X_train,y_train,X_test,y_test,xg_g=0,trial=None,num_epochs=0,alpha=0.015):
  ...
  if trial==None:
    ...
    plt.savefig("/home2/username/legal-tech/doc2vec-adw-xg.png",dpi=300)
    with open("./doc2vec-adw-xg.pkl","wb") as f:
        pickle.dump(clf,f)
    model.save("./doc2vec-adw-xg.model")
...
with open(f"/scratch/username/train_test_data_for_modelling/test_split_alldistricts.json") as f:
    data = json.load(f)
...
with open(f"/scratch/username/train_test_data_for_modelling/train_split_alldistrict_bail.json") as f:
    data = json.load(f)
    df = pd.DataFrame(data)
    #df = df.head(100)
    df2 = df.sample(frac = 0.1, random_state=42).reset_index()
...
with open("/home2/username/legal-tech/doc2vec-adw-xg.json","w") as f:
