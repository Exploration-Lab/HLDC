...
with open("/scratch/arjunth2001/train_test_data_for_modelling/train_split_44_districts.json") as f:
    data = json.load(f)
df_train = pd.DataFrame(data)


with open("/scratch/arjunth2001/train_test_data_for_modelling/validation_split_10_districts.json") as f:
    data = json.load(f)
...
training_args = TrainingArguments(
    output_dir='/scratch/username/results_first_512_parameter_district_wise',          # output directory
    num_train_epochs=5,              # total number of training epochs
    per_device_train_batch_size=8,  # batch size per device during training
    per_device_eval_batch_size=8,   # batch size for evaluation
    warmup_steps=500,                # number of warmup steps for learning rate scheduler
    weight_decay=0.01,               # strength of weight decay
    logging_dir='/scratch/username/logs_first_512_parameter_district_wise',            # directory for storing logs
...
final_trainer.train()
final_trainer.save_model("/home2/username/courts/final_models/first_512_district_wise")