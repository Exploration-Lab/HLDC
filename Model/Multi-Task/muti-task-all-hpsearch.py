...
def eval_func_epoch(model, dataloader, device, epoch):
    ...
    if epoch == "TESTING":
        ...
        plt.savefig("/home2/username/legal-tech/mt-all.png",dpi=300)
...
num_warmup_steps = 0
save_model = True 
model_path = "./mt-all.pt" ## Change after every experiment
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
import json
with open("/home2/username/legal-tech/mt-all.json","w") as f:
