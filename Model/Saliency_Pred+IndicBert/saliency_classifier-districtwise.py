...
base_path="/scratch/username_s/saliency_summaries/"
...
def clean_dataset():
    train = pd.read_csv(f'{base_path}train_split_44_districts.csv')
    test = pd.read_csv(f'{base_path}test_split_17_districts.csv')
    val = pd.read_csv(f'{base_path}validation_split_10_districts.csv')
...
def eval_func_epoch(model, dataloader, device, epoch):
    ...
    if epoch == "TESTING":
        ...
        plt.savefig("/home2/username/legal-tech/sc-dw.png",dpi=300)
...
num_warmup_steps = 0
save_model = True 
model_path = "./sc-dw.pt" ## Change after every experiment
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
import json
with open("/home2/username/legal-tech/sc-dw.json","w") as f:
