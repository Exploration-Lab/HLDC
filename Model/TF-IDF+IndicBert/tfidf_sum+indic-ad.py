...
train_df = pd.read_csv("/scratch/username/tfidf_summaries/train_split_alldistrict_bail.csv")
test_df = pd.read_csv("/scratch/username/tfidf_summaries/val_split_alldistrict.csv")
...
training_args = TrainingArguments(
    output_dir='/scratch/username/htf2_results',          # output directory
    num_train_epochs=5,            # total number of training epochs
    per_device_train_batch_size=8,  # batch size per device during training
    per_device_eval_batch_size=8,   # batch size for evaluation
    warmup_steps=500,               # number of warmup steps for learning rate scheduler
    weight_decay=0.01,              # strength of weight decay
    logging_dir='/scratch/username/htf2_logs',           # directory for storing logs
