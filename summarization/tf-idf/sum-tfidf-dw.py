...
try:
    os.mkdir("/scratch/username/summaries_octtfidf")
except:
    pass
...
files = ["train_split_44_districts.json"]
for file in files:
    put_in_corpus(file)
...
def process(file):
    ...
    df.to_csv(f"/scratch/username/summaries_octtfidf/{file}")


# In[ ]:


files = ["train_split_44_districts.json","test_split_17_districts.json","validation_split_10_districts.json"]
for file in files:
  process(file)
